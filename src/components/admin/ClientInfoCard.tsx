...
export function ClientInfoCard({ client, plans }: ClientInfoCardProps) {
    ...
    const handleSave = async () => {
        ...
        if (Object.keys(updateData).length > 0) {
            await dispatch(updateClient({ id: client.id, data: updateData }));
        }
        setIsEditing(false);
    };
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    {isEditing ? (
                        <>
                            ...
                            <div className="space-y-2">
                                ...
                                <Input
                                    id="planRenewalDate"
                                    type="date"
                                    value={formData.planRenewalDate ? new Date(formData.planRenewalDate).toISOString().split('T')[0] : ''}
                                    onChange={(e) => setFormData({ ...formData, planRenewalDate: e.target.value })}
                                />
                            </div>
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setFormData({ ...formData, remainingMessages: parseInt(e.target.value) })}
                                />
                            </div>
                        </>
                    ) : (
                        <>
                            ...
                            <div>
                                <p className="text-sm font-medium text-muted-foreground">Plan Renewal Date</p>
                                <p className="text-lg">{formatDate(client.planRenewalDate)}</p>
                            </div>
                            <div>
                                <p className="text-sm font-medium text-muted-foreground">Remaining Messages</p>
                                <p className="text-lg">{client.remainingMessages?.toLocaleString() || 'Not set'}</p>
                            </div>
                            <div>
