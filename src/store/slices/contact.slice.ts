...
export const searchContacts = createAsyncThunk(
  ...
  async (
    ...
    try {
      return await contactService.search({ q, cursor, limit });
    } catch (error: any) {
      return rejectWithValue(
...
export const updateContact = createAsyncThunk(
  ...
  async (
    { id, data }: { id: string; data: Partial<CreateContactDTO> },
    { rejectWithValue }
  ) => {
    try {
...
export const bulkUploadContacts = createAsyncThunk(
  "contacts/bulkUpload",
  async (
    { file, groupId }: { file: File; groupId?: string },
    { rejectWithValue }
  ) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Failed to upload contacts"
      );
    }
  }
);
...
const contactSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(searchContacts.pending, (state, action) => {
      // Only show full-screen loader on fresh searches (no cursor)
      if (!action.meta.arg.cursor) {
    ...
    builder.addCase(searchContacts.fulfilled, (state, action) => {
      ...
      if (action.meta.arg.cursor) {
        // Load more within search results — append
        state.items = [...state.items, ...action.payload.data];
      } else {
    ...
    builder.addCase(searchContacts.rejected, (state, action) => {
      state.isLoading = false;
      state.isSearching = false;
      state.error = action.payload as string;
    });
    ...
    builder.addCase(bulkDeleteContacts.fulfilled, (state, action) => {
      ...
      if (
        ...
        action.payload.includes(state.selectedContact.id)
      ) {
...
export const selectSelectedContact = (state: { contacts: ContactState }) =>
  state.contacts.selectedContact;
export const selectContactLoading = (state: { contacts: ContactState }) =>
  state.contacts.isLoading;
export const selectIsSearching = (state: { contacts: ContactState }) =>
  state.contacts.isSearching;
export const selectSearchQuery = (state: { contacts: ContactState }) =>
  state.contacts.searchQuery;
export const selectContactError = (state: { contacts: ContactState }) =>
  state.contacts.error;
export const selectCustomFieldConfig = (state: { contacts: ContactState }) =>
  state.contacts.customFieldConfig;
export const selectNextCursor = (state: { contacts: ContactState }) =>
  state.contacts.nextCursor;
export const selectUploadResult = (state: { contacts: ContactState }) =>
  state.contacts.uploadResult;
export const selectIsUploading = (state: { contacts: ContactState }) =>
  state.contacts.isUploading;
