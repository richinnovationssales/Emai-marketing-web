...
export const clientService = {
    ...
    getAll: async (): Promise<ClientWithStats[]> => {
        const { data } = await apiClient.get<ApiResponse<ClientWithStats[]>>(API_ENDPOINTS.CLIENTS.BASE);
        return data.data;
    },

    // Get pending clients
    getPending: async (): Promise<ClientWithStats[]> => {
        const { data } = await apiClient.get<ApiResponse<ClientWithStats[]>>(API_ENDPOINTS.CLIENTS.PENDING);
        return data.data;
    },
    ...
    create: async (clientData: CreateClientDTO): Promise<Client> => {
        const { data } = await apiClient.post<ApiResponse<Client>>(API_ENDPOINTS.CLIENTS.BASE, clientData);
        return data.data;
    },

    // Update client
    update: async (id: string, clientData: UpdateClientDTO): Promise<Client> => {
        const { data } = await apiClient.patch<ApiResponse<Client>>(API_ENDPOINTS.CLIENTS.BY_ID(id), clientData);
        return data.data;
    },
    ...
    deactivate: async (id: string): Promise<Client> => {
        const { data } = await apiClient.patch<ApiResponse<Client>>(API_ENDPOINTS.CLIENTS.DEACTIVATE(id));
        return data.data;
    },

    // Reactivate client
    reactivate: async (id: string): Promise<Client> => {
        const { data } = await apiClient.patch<ApiResponse<Client>>(API_ENDPOINTS.CLIENTS.REACTIVATE(id));
        return data.data;
    },
    ...
    getAnalytics: async (id: string): Promise<ClientAnalytics> => {
        const { data } = await apiClient.get<ApiResponse<ClientAnalytics>>(API_ENDPOINTS.CLIENTS.ANALYTICS(id));
        return data.data;
    },

    // Onboard client
    onboard: async (onboardData: OnboardClientDTO): Promise<Client> => {
        const { data } = await apiClient.post<ApiResponse<Client>>(API_ENDPOINTS.CLIENTS.ONBOARD, onboardData);
        return data.data;
    }
};
